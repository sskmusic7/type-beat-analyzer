...
class AudioDNA:
    ...
    def profile(self, audio_path: str) -> Dict[str, Any]:
        ...
        # Phase 3: Stem separation — NOT available on Cloud Run (training-only)
        # StemSeparator requires demucs which is not deployed to Cloud Run.
        if self.enable_stems:
            print("[AudioDNA] Stem separation not available on Cloud Run, skipping.")
            result["stems"] = None
    ...
    def to_vector(self, dna: Dict[str, Any]) -> List[float]:
        ...
        Vector layout (83 dims):
            [0:52]   CLAP scores x 0.3
            [52:53]  BPM normalized x 4.0
            [53:54]  Key confidence x 3.0
            [54:56]  Spectral (centroid, flatness) x 2.0
            [56:76]  MFCC means x 3.0
            [76:77]  RMS energy x 3.0
            [77:79]  Rhythm (onset density, beat regularity) x 3.0
            [79:83]  Stem mix ratios x 2.5
        """
